...
import { InputField, ComboboxSuggestion } from '../elements/index.js';
...
const AutoCompleteSuggestionsContainer = styled.div`
    ...
    left: 0;
    right: 0;
    max-height: calc(${props => props.visibleItems} * ${props => props.itemHeight});
    overflow: hidden;
    overflow-y: auto;
`;
...
export default class Combobox extends React.Component {
    ...
    onKeyDownHandler(e) {
        ...
        const itemHeightAsNumber = parseInt(itemHeight, 10);
        switch (e.keyCode) {
            ...
            case KeyboardKeys.KEY_DOWN:
                this.setState((prevState) => ({
                    ...
                    if ((this.state.currentFocus + 1) * itemHeightAsNumber > ((visibleItems * itemHeightAsNumber) + this.suggestions.current.scrollTop)) {
                        this.suggestions.current.scrollTop += itemHeightAsNumber;
                    }
                    if (this.state.currentFocus * itemHeightAsNumber < this.suggestions.current.scrollTop) {
            ...
            case KeyboardKeys.KEY_UP:
                this.setState((prevState) => ({
                    ...
                    if ((this.state.currentFocus + 1) * itemHeightAsNumber > ((visibleItems * itemHeightAsNumber) + this.suggestions.current.scrollTop)) {
                         this.suggestions.current.scrollTop = this.suggestions.current.querySelectorAll('div').length * itemHeightAsNumber;
                    }
                });
                break;
